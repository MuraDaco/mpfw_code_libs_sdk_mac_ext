...
dtyCntnrArraySet_t::dtyCntnrArraySet_t   (tuiGraphicUnit_t* p_array) :
     g_array        {p_array}
    ,g_y0r          {0}
{
    resetSelectElement();
...
void dtyCntnrArraySet_t::resetSelectElement         (void)    {
    uint8_t l_id = 0;
...
bool dtyCntnrArraySet_t::resetLoopElement         (void)    {
...
// bool dtyCntnrArraySet_t::bLoopInitDisplay           (uint8_t p_id, void* p_pParent)    {
//     bool l_result = false;
...
uint8_t dtyCntnrArraySet_t::getLoopInitCycles       (void)    {
    return g_arraySize;
}
...
void dtyCntnrArraySet_t::initDisplay                (uint8_t p_id, void* p_pParent)  {
    g_poParent = static_cast<tuiGraphicUnit_t*>(p_pParent);

    // init "g_loopY0rElement" global parameter
    g_loopBoundYtop     = (p_id) ? g_loopBoundYtop      : 0;
    g_loopBoundYbottom  = (p_id) ? g_loopBoundYbottom   : 0;
    ...
    // determine the "g_loopY0rElement" (global parameter) for the next cycle
    g_loopBoundYtop     = MIN(g_loopBoundYtop,      g_array[p_id].getRelCoordY());
    g_loopBoundYbottom  = MAX(g_loopBoundYbottom,   g_array[p_id].getRelCoordY() + g_array[p_id].getDimH() - 1);

    g_h = g_loopBoundYbottom - g_loopBoundYtop + 1;
}

#define RECURSIVELY     true
bool dtyCntnrArraySet_t::selectElementByMouse      (void)      {
    bool l_result = false;
...
int32_t dtyCntnrArraySet_t::getDeltaShiftBySelect           (void)    {
    return g_array[g_selectIdElement].getDistanceFromBound();
}

void dtyCntnrArraySet_t::shiftLoopElementBySelect       (int32_t p_delta)    {

    if(p_delta) g_array[g_loopIdElement].incRelCoordY(p_delta);
}

void dtyCntnrArraySet_t::updSelectElement       (void)    {
    g_array[g_selectIdElement].deselectBackNselect();
}

void dtyCntnrArraySet_t::clearDisplayBox       (void)    {
    g_poParent->clear();
}

bool dtyCntnrArraySet_t::updCntnrRelCoord           (int32_t p_delta)    {
    bool l_result = true;
    g_y0r += p_delta;
    if(g_y0r < 0)   {
        g_y0r -= p_delta;
...
void dtyCntnrArraySet_t::shiftLoopElementRollUp       (void)    {
...
void dtyCntnrArraySet_t::shiftLoopElementRollDown       (void)    {
    g_array[g_loopIdElement].incRelCoordY(1);
}

void dtyCntnrArraySet_t::updElementCoordNbounds       (void)    {
    g_array[g_loopIdElement].updParamsAfterParentMod();
}

void dtyCntnrArraySet_t::dspElement                 (bool p_recursively)  {
    g_array[g_loopIdElement].display(p_recursively);
}

bool dtyCntnrArraySet_t::nextLoopElement          (void)    {
    bool l_result = false;
    g_loopIdElement++;
...
void* dtyCntnrArraySet_t::getSelectedItem               (void)      {
    return &g_array[g_selectIdElement];
}

bool dtyCntnrArraySet_t::setSelectPrev                  (void)    {
    bool l_result = false;
    if(g_arraySize)   {
...
bool dtyCntnrArraySet_t::setSelectNext                  (void)    {
    bool l_result = false;
    if(g_arraySize)   {
...
bool dtyCntnrArraySet_t::bSelectVisibleCompletely       (void)    {
