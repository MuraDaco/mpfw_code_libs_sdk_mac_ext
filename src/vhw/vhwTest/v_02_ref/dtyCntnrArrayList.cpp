...
dtyCntnrArrayList_t::dtyCntnrArrayList_t   (tuiGraphicUnit_t* p_array) :
     g_array        {p_array}
    ,g_y0r          {0}
{
    resetSelectElement();
...
void dtyCntnrArrayList_t::resetSelectElement         (void)    {
    uint8_t l_id = 0;
...
bool dtyCntnrArrayList_t::resetLoopElement         (void)    {
...
// bool dtyCntnrArrayList_t::bLoopInitDisplay           (uint8_t p_id, void* p_pParent)    {
//     bool l_result = false;
...
uint8_t dtyCntnrArrayList_t::getLoopInitCycles       (void)    {
    return g_arraySize;
}

void dtyCntnrArrayList_t::initDisplay                (uint8_t p_id, void* p_pParent)  {
...
bool dtyCntnrArrayList_t::selectElementByMouse      (void)      {
    bool l_result = false;
...
int32_t dtyCntnrArrayList_t::getDeltaShiftBySelect           (void)    {
    // determine the delta
    return g_array[g_selectIdElement].getDistanceFromBound();
}

void dtyCntnrArrayList_t::shiftLoopElementBySelect       (int32_t p_delta)    {

    if(p_delta) g_array[g_loopIdElement].incRelCoordY(p_delta);
}

void dtyCntnrArrayList_t::updSelectElement       (void)    {
    g_array[g_selectIdElement].deselectBackNselect();
}

void dtyCntnrArrayList_t::clearDisplayBox       (void)    {
    g_poParent->clear();
}

bool dtyCntnrArrayList_t::updCntnrRelCoord           (int32_t p_delta)    {
    bool l_result = true;
    g_y0r += p_delta;
    if(g_y0r < 0)   {
        g_y0r -= p_delta;
...
void dtyCntnrArrayList_t::shiftLoopElementRollUp       (void)    {
...
void dtyCntnrArrayList_t::shiftLoopElementRollDown       (void)    {
    g_array[g_loopIdElement].incRelCoordY(1);
}

void dtyCntnrArrayList_t::updElementCoordNbounds       (void)    {
    g_array[g_loopIdElement].updParamsAfterParentMod();
}

void dtyCntnrArrayList_t::dspElement                 (bool p_recursively)  {
    g_array[g_loopIdElement].display(p_recursively);
}

bool dtyCntnrArrayList_t::nextLoopElement          (void)    {
    bool l_result = false;
    g_loopIdElement++;
...
void* dtyCntnrArrayList_t::getSelectedItem               (void)      {
    return &g_array[g_selectIdElement];
}

bool dtyCntnrArrayList_t::setSelectPrev                  (void)    {
    bool l_result = false;
    if(g_arraySize)   {
...
bool dtyCntnrArrayList_t::setSelectNext                  (void)    {
    bool l_result = false;
    if(g_arraySize)   {
...
bool dtyCntnrArrayList_t::bSelectVisibleCompletely       (void)    {
