...
tuiBaseWlistUnit_t::tuiBaseWlistUnit_t      (const char* p_strName, box_t* p_box, zone_t*    p_zoneList, element_t* p_elementList) :
...
void tuiBaseWlistUnit_t::initElementsList       (void) 	{
...
void tuiBaseWlistUnit_t::initElementsList       (point_t p_point0) 	{
    if(!g_elementList)   return;
    // c) initialize all elements of the its own element list that is
    element_t* l_element = g_elementList;
...
bool tuiBaseWlistUnit_t::selectElements     (void)    {
    bool l_result = false;
...
void tuiBaseWlistUnit_t::prevElement     (tuiBaseWlistUnit_t* p_po)    {
    if(p_po->g_bElementList) {
        ...
        p_po->g_pCurrentElement->element->selectX(p_po);
    } else
        mvwprintw(p_po->g_ncursWin, 4, 5, "NO ELEMENTS");
}

void tuiBaseWlistUnit_t::nextElement	(tuiBaseWlistUnit_t* p_po)  {
    if(p_po->g_bElementList) {
        if(p_po->g_pCurrentElement) {
            p_po->g_pCurrentElement++;
...
void tuiBaseWlistUnit_t::refreshElements     (void)    {
    element_t* l_element = g_elementList;
