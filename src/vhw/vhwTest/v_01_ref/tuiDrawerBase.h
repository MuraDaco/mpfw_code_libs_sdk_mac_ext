 ...
 * tuiDrawerBase.h
 *
 *  Created on: Aug, 5rd 2024
 *      Author: Marco Dau
 */
 
#ifndef TUI_DRAWER_BASE_H
#define TUI_DRAWER_BASE_H
...
#define tuiDrawer__name(y, x)       \
        mvwprintw(g_pNcursWin, g_y0 + y, g_x0 + x, " *~ %s ~* ", tuiDrawerBase_t::g_strName);     wrefresh(g_pNcursWin)

class tuiDrawerBase_t   {

public:
    tuiDrawerBase_t (box_t *p_pBox);
    tuiDrawerBase_t (const char* p_strName, box_t *p_pBox);
    tuiDrawerBase_t (WINDOW* p_pNcursWin, box_t *p_pBox);
    tuiDrawerBase_t (uint8_t p_h, uint8_t p_w, uint8_t p_y0, uint8_t p_x0);
    tuiDrawerBase_t (const char* p_strName, box_t p_box, dtyUint8_t* p_pDtyStatus);
